...
| 결제        | **Stripe** (구독 + 일회성)                                                           | —                                                 |
| AI: 텍스트  | **Anthropic Codex (Codex-sonnet-4-6, 캐시 활성)**                                    | 대본 분석/태깅/추천                               |
...
본 프로젝트는 **Codex(코드 작성/리뷰) + Codex(디자인 비주얼/카피) + Cursor(IDE 보조 편집) + nano-banana 2(이미지 생성)** 를 조합해 진행한다.

### 7.1 도구 역할 분담

| 도구                       | 1차 역할                                                            | 2차 역할             |
| -------------------------- | ------------------------------------------------------------------- | -------------------- |
| **Codex (이 환경)**        | 코드 생성/리팩터, 모듈 설계, 문서 유지, 서브에이전트 오케스트레이션 | PR 작성, 테스트 생성 |
| **Codex(아티팩트/디자인)** | UI 시안, 디자인 비평, UX 카피                                       | 마케팅 카피          |
| **Cursor**                 | 휴먼이 직접 미세 편집, 인라인 리뷰                                  | 큰 파일 빠른 그렙    |
| **nano-banana 2**          | 배경/소품/꾸미기 보조 이미지 생성, 썸네일 변환                      | OG 이미지 자동 생성  |

### 7.2 오토파일럿 루프 (Codex 주도)
...
- 각 서브에이전트의 시스템 프롬프트는 `.Codex/agents/*.md` 에 고정.
