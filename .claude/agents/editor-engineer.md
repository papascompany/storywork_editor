...
```bash
pnpm --filter @storywork/<pkg> lint   # warning 0 (--max-warnings 0 강제)
pnpm --filter @storywork/web build    # next build (ESLint 포함) — editor-* 변경 시 web build 검증 필수
pnpm --filter @storywork/<pkg> test
```
...
- `pnpm --filter <pkg> lint` 통과해도 `next build` 에서 별도로 거부될 수 있으므로 **반드시 web build 도 실행**
- `editor-*` 패키지는 React/DOM import 금지 (`editor-ui` 제외)
