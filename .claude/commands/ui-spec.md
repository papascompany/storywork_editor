...
## 진입 전 필수 확인 (회고 §7.4 Step 0 — FOLLOWUP-58)

`/ui-spec` 으로 명세화를 시작하기 **전에** 반드시:

1. prod deploy 가 최신 commit 빌드인지 (`vercel ls <project> --limit 3` 로 state=READY + SHA 일치 확인)
2. 변경한 utility 가 prod CSS 에 실제로 있는지 (`bash scripts/check-prod-sanity.sh --web --utility <class>`)
3. 둘 다 OK 면 아래 절차로 진행. 아니면 `docs/process/ui-feedback-workflow.md §2 Step 0` 우선 처리

이 단계를 생략하면 회고 §3 의 13일 spacing 실패 패턴(코드 변경했는데 prod 에 안 보임) 재발 위험.
