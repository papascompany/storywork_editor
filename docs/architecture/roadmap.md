...
- [ ] **[FOLLOWUP-55] (P0) CSS utility 빌드 sanity CI gate** — 회고 §5.1. `scripts/css-sanity-check.sh` — 빌드 산출물 `apps/*/next/static/css/*.css` 에 `.p-4/p-5/p-8/gap-4/gap-5/gap-8/px-N/py-N` 등 핵심 utility rule 존재 검증 + universal `* { padding: 0 }` anti-pattern 검출. CI job 으로 lint 다음 단계에 추가. 본 검증이 있었다면 13일 spacing 작업이 첫 commit 30초 안에 끝났을 것 — @architect
- [ ] **[FOLLOWUP-56] (P0) ADR-0012 Tailwind v4 monorepo @source policy** — `packages/shared-ui/src/styles/globals.css` 의 `@source` directive 가 monorepo 모든 source 경로(apps/web, apps/admin, packages) 를 명시해야 utility 가 생성됨. 신규 패키지에 className 사용처 추가 시 `@source` 갱신 의무. ADR 추가 + CI 에서 `@source` 누락 검증 — @architect
- [ ] **[FOLLOWUP-57] (P1) ESLint custom rule: universal `*` + padding/margin 차단** — `packages/shared-ui/src/styles/globals.css:227` 류 anti-pattern 재발 방지. `stylelint` 또는 PostCSS plugin 으로 layer 밖 `*` selector + box-model 속성 0 reset 검출 → 회고 Layer 2 차단 — @architect
...
- [ ] **[FOLLOWUP-54] (P1) visual-check 인프라 한계 보강** — 본 4a7cea2 작업에서 발견. (a) Playwright headless 가 Next.js dev 서버의 Tailwind `md:` breakpoint 를 적용하지 못해 FeatureSidebar `width: 0px` 로 렌더됨 → SSR/hydration 타이밍 또는 matchMedia mock 누락 의심. (b) `visual-check.sh` 가 localhost 전용이라 Vercel prod 캡처 불가 — 일회용 inline Playwright 로 우회함. 산출: `--url <full>` 옵션 추가, `--click <selector>` interaction 옵션 추가, dev 서버 headless 에서 `md:` 정상 동작 패치. DoD: `pnpm visual-check /editor --click "[aria-label='포즈']"` 가 PosePanel 열린 상태로 캡처 — @architect
- [ ] **[FOLLOWUP-49] (P1) `apps/web` typecheck + test 부채** — typecheck 21+ TS 에러 (editor-template/editor-text dist export 형태, `__tests__/lib/users.test.ts` mock 타입, `marketing/Header.tsx` 의 `@/lib/supabase/client` path alias resolve 실패). test 5+ FAIL (marketing/* vite alias, page-persistence/command-palette/editor.smoke localStorage jsdom 환경). 현재 두 script 모두 echo 로 명시 스킵 (CI green 위해). admin/packages 는 그대로 게이트 — @architect + @editor-engineer
- [ ] **[FOLLOWUP-50] (P1) `apps/storybook` Storybook 8.x type migration** — AdminAuditLog/AdminDataTable 외 다수 stories 가 `args` 누락 (Story type strict). MarketingStickyNote 의 Header.tsx prop 의존도. 현재 typecheck + build script 모두 echo 로 명시 스킵 (CI green 위해). 모든 stories 의 default args 일괄 추가 후 typecheck/build 복원 — @ui-designer
