...
- [ ] **[FOLLOWUP-65] (P1) prisma 마이그레이션 baseline 부재 — fresh DB `migrate deploy` 실패** — 핵심 도메인 테이블(User/Format/Resource/Project/Page/Showcase/ContestSeason/TemplateSet 등)이 어떤 마이그레이션에서도 `CREATE TABLE` 되지 않고, 과거 `prisma db push` 로 만든 baseline 위에 hand-written ALTER 만 쌓는 관행. 가장 이른 마이그레이션(`20260507000000_add_admin_role_totp`)부터 `ALTER TABLE "User"` 를 테이블 생성 없이 수행 → 빈 DB(CI/신규 스테이징/DR)에서 `prisma migrate dev|deploy` 가 "relation does not exist" 로 실패. 마이그레이션 히스토리 재현 불가(drift). **BOARD-05 적대적 리뷰에서 발견(선존 repo-wide, 특정 PR 무관)**. 복구: `prisma db pull` → `migrate diff --from-empty` baseline init 추가 → `migrate resolve --applied` 로 prod 정합화 → CI 에 빈 DB `migrate deploy` 스모크 추가 — @architect 🚦(prod 정합화 휴먼 게이트)
