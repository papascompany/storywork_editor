...
- [ ] [FOLLOWUP-68] Reaction.userId FK 부재 — Reaction.userId 가 plain String(User FK 없음). 현재 hard-delete cron 이 앱레벨 deleteMany 로 방어하나, 향후 다른 User 삭제 경로 대비 `user User @relation(onDelete:Cascade)` 추가 = 마이그레이션 — @architect 🚦(휴먼게이트)
- [ ] [FOLLOWUP-69] 탈퇴(deletedAt) 차단 defense-in-depth — 현재 web middleware 단일 레이어(Supabase REST 장애 시 fail-open). 서버 컴포넌트/`/api/projects/*` 레이어에 deletedAt 재검증 가드 추가 — @architect
- [ ] [FOLLOWUP-70] 감사 잔여 하드닝(verifier 가 low~medium 하향, 저우선) — Page.fabricJson save 시 `PageJsonV1Schema` 검증 / admin `next.config` remotePatterns `*.supabase.co` → 호스트 핀 고정 / 비밀번호 정책 강화(길이↑·복잡도·HIBP) / report 상태머신 가드(터미널 상태 409) / env.ts CRON_SECRET 스키마+부트 배선 / CSS anti-pattern 체커 in_layer 단순화 — @architect
