...
- [x] [MODE-04b] 웹툰 세로 스트립 편집 UI — @editor-engineer ✅ 2026-08-10.
  · `editor-core/src/strip/geometry.ts` 신설(React 무의존 순수 산술): `stripLayout`(간격 누적·마지막 간격 제외) · `visibleRange`(overscan·간격 폴백) · `segmentAtScroll` · `scrollToSegment` · `fitStripWidth`. `StoryCanvas.setViewportScale()` 로 뷰포트 API 를 코어에 회수(종전 web 이 `_fabricCanvas` 로 직접 조작)
  · web `webtoon/` 3종: `useStripVirtualizer`(라이브 창 = 데스크톱 3·모바일 2, **히스테리시스** 진입 좁게/이탈 넓게로 경계 플리커 차단, **활성 세그먼트 pin** 으로 편집 중 선택·히스토리 보호) · `StripSegmentCanvas`(언마운트 전 flush → dispose 순서 고정) · `StripCanvas`(스크롤 컨테이너 + 절대 배치 슬롯, 비라이브는 썸네일)
  · **선행 조건 해소**: `usePageStore.updatePageJson(index, json)` 신설 — 현행 `updateCurrentPageJson` 은 캔버스 N개 환경에서 다른 세그먼트를 덮어쓴다
  · EditorShell 분기는 **판형 단위계가 곧 조건**(`format.unit === 'px'`) — 별도 플래그를 두면 판형과 어긋나 mm 프로젝트가 스트립으로 열린다
  · **실측 함정**: 가상화 훅이 `RefObject` 를 받으면 effect 의존성이 불변이라 마운트 시 `current` 가 null 이었을 때 **영영 재시도하지 않는다**(컨테이너 623px 인데 훅은 0 으로 알고 스크롤 미추종). callback ref + state 로 엘리먼트를 직접 받게 교정
...
- [x] [LAYOUT-05] 구도 균형 개선 (LAYOUT-04 실측 후속) — @layout-composer ✅ 2026-08-08.
  · **종합 79.3% → 87.6%** · 구도 균형 50.7→61.5 · 지면 밀도 62.5→90.2(†측정 정확화 포함) · 얼굴 가림 97.0→**100%** · 시선 흐름 100 유지 · **페이지 222 유지** · 채택률/보존 100% 유지
  · **진단이 로드맵 가설("1on1-talk 포즈 기하부터")을 뒤집음** — 1on1-talk 는 dist 0.110 으로 양호했고, 범인은 **preset-wide 61페이지 전원 balance 0점**(dist 0.403, 우상단 쏠림). 원인 = ①생성 격자의 "위에서 아래" 채움 ②wide 기존 말풍선의 좌상단 고정
  · `bubble-slots.ts` — 생성 슬롯 채택을 3단 판정으로: **①요청 수 충족 → ②미래 자리 보존 → ③중앙 근접**. 중앙 우선 단독은 페이지 222→264(+19%, 행당 2개→1개), 요청 수 우선만 추가하면 261(중앙 열 세로 점유가 캡션 자리를 전멸시켜 screenplay 지문이 페이지로 격리 51→94p) — 두 부작용 모두 실측으로 잡고 3단으로 해소
  · `shrinkAwayFromFaces` — 격자 후보가 얼굴과 겹치면 얼굴 x-구간을 뺀 최대 빈 구간으로 축소(최소 폭 0.22). 잔여 얼굴 가림 3% 제거
  · wide 말풍선 좌상단 → 인물 위 중앙(x0.25·y0.26) · `quality.ts unionArea` 좌표 압축 합집합(밀도 과대 추정 제거, 페이지 밖 클램프)
  · 균형 축은 이제 **부분 결합**(생성 슬롯 동률 해소에만 관여) — quality.ts 주석에 명시
  · 신규 테스트 12(`layout05.test.ts`), ai-layout 248 green. 리포트 [docs/eval/layout-quality-2026-08-08.md](../eval/layout-quality-2026-08-08.md)
- [ ] [LAYOUT-06] 배치 품질 잔여 축 개선 — 구도 균형 61.5%(wide 잔여 dist 0.215 — shrink 의 좌측 편향, 슬롯 좌우 대칭 쌍 생성 검토) · 대사 밀도 86.5%(캡션 전용 페이지를 축 정의에서 제외할지) · 1on1-talk 캡션 수용량 대사 3~4 구간 −1(실측 페이지 영향 0). **수확 체감 구간 — 실사용 로그(FOLLOWUP-60) 도입 후 실측 채택률과 상관을 보고 재개** — @layout-composer + @qa-tester
