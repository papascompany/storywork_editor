...
## ADR-0016 — 사업화 BM: 교육·디자인마켓의 '공정 외부화' + AI 실가동 시퀀스 (2026-06-29)

- **결정**: 사업화(수익) 모델을 다음과 같이 정한다.
  - **기반 수익**: POD 인쇄 출판 + 크리에이터 구독(마이데이터).
  - **확장 수익**: 교육 커리큘럼(B2B 기관·B2C 온라인) + 디자인 마켓플레이스(외부 디자이너 자산 거래, 수수료 25%).
  - **포지셔닝 원칙**: 교육·마켓은 별도 플랫폼/에듀테크 신사업이 아니라 **"AI로 혁신한 자사 제작공정의 외부화"** 로 서술한다. (그랜트/투자 평가에서 '제품 R&D·플랫폼 창업'으로 재분류되어 취지 이탈 컷오프되는 것을 방지)
- **AI 실가동 시퀀스 재정의**: 이미 구축된 기반(M2 인덱싱·M4 대본분석/자동배치·M6 POD) 위에서 '신규 개발'이 아닌 **'실가동·정밀화'** 로 순서를 재정의 → ① mock→실 임베딩 ② AI 2차 태깅·골든셋 확장 ③ 라이브 LLM·채택률 재측정 ④ 교육·마켓 온보딩. ([roadmap SW-BIZ](roadmap.md))
- **수익 시뮬레이션(보수)**: Y1 ≈ 5,300만 → Y2 ≈ 2.0억 → Y3 ≈ 4.6억 (단위 만원). 현재 유료고객 0(결제 미구현)에서 출발, 결제·온보딩 가동 전제. top-line만(인건비 선행 별도), 마켓 GMV는 공급자 온보딩에 강하게 연동.
- **맥락**: 「2026 혁신 소상공인 AI 활용지원사업」 사업계획 정립 과정에서 다관점 평가패널(평가위원·시장·AI전략·소상공인적합성·적대검토)로 도출. 신청 주체 (주)보물상자 = '콘텐츠 제작 소상공인'.
- **휴먼 게이트**: 결제(Stripe)·수수료/가격·정산(세무)·교육 데이터모델 마이그레이션은 모두 🚦.
- **결정일**: 2026-06-29

---

## ADR-0017 — 산출물 모드: 웹툰 / POD 를 프로젝트 생성 시 선택 (2026-08-07)

- **결정**: 웹툰과 POD 중 **하나를 제품 우선순위로 정하지 않는다.** 대신 사용자가 **프로젝트를
  시작할 때 모드를 고른다**(`Project.mode: 'webtoon' | 'pod'`). 두 모드는 같은 편집 코어와 같은
  AI 파이프라인(분석 → 추천 → 배치)을 쓰고, **판형·페이지 개념·출력**에서만 갈라진다.

| | POD 모드 (현행) | 웹툰 모드 (신규) |
|---|---|---|
| 판형 | mm · dpi · bleed · safe | px 비율 (예: 폭 690~800, 세로 가변) |
| 지면 단위 | Page(고정 크기) | 세로 스트립(에피소드) — 컷 사이 간격 |
| 배치 제약 | 재단선·안전영역 | 스크롤 리듬 · 컷 간 여백 |
| 출력 | POD PDF | 이미지 시퀀스 / 단일 롱 이미지 |

- **맥락**: 현재 아키텍처는 POD 전제다 — `Format` 이 mm/dpi/bleed/safe 이고, `ai-layout` 은
  **mm 좌표 전용**(CLAUDE.md §8)이며 `page-split`/`capacity` 가 "고정 크기 페이지에 담기는가"로
  계획한다. 웹툰은 세로 길이가 사실상 무한이라 "담기는가"의 의미 자체가 다르다 —
  수용량 초과가 페이지 분할이 아니라 **스트립 연장**이 된다.
- **파급**: 판형 프리셋 추가만으로 끝나지 않는다. (a) `LayoutFormat` 에 단위 개념 도입,
  (b) `capacity` 의 분할 전략 분기, (c) `editor-core` 세로 무한 캔버스, (d) `editor-export`
  이미지 시퀀스 경로. 그래서 **이번에는 결정과 작업 분해까지만** 하고 구현은 별도로 잡는다
  ([roadmap MODE](roadmap.md)).
- **하지 않는 것**: 기존 POD 경로의 mm 전용 원칙은 유지한다. 웹툰 모드가 mm 를 px 로 바꾸는 게
  아니라, `Format` 이 두 단위계를 **구분해서** 갖는다 — 섞으면 인쇄 사고가 난다.
- **결정일**: 2026-08-07 (대표 결정)

---

## ADR-0018 — 디자인 마켓: AI 생성 에셋 허용 + 생성 표기 의무 (2026-08-07)

- **결정**: AI 로 생성한 이미지의 마켓 등록을 **허용**한다. 단 등록 시 **생성 여부와 사용 모델을
  메타로 받고 상세 페이지에 표시**한다(구매자 알 권리). 자사 생성기(nano-banana) 산출물과 외부
  도구 산출물을 구분하지 않는다.
- **이유**: 공급자 온보딩 목표(Y1 30명 → Y3 300명, [ADR-0016](#adr-0016--사업화-bm-교육디자인마켓의-공정-외부화--ai-실가동-시퀀스-2026-06-29))
  대비 전면 금지는 공급 규모를 크게 줄이고, 사람이 그렸는지를 검증할 실효적 수단도 없다.
  전수 사람 검수는 운영 인력이 선형으로 늘어 소상공인 규모에 맞지 않는다.
- **함께 지킬 것**:
  - 등록 폼 필수 필드: `isAiGenerated` · `generatorModel` · 상업 사용 가능 여부
  - 기존 라이선스 강제(사이드카 `license` 누락 시 적재 거부, [ADR-0011b](#adr-0011b))와 동일하게
    **누락 시 등록 거부**
  - 타인 저작물 학습·모사 신고 채널 + 사후 내림(takedown) 절차는 마켓 오픈 전 필수
- **미결(별도 결정)**: 생성 에셋의 수수료율을 손그림과 동일한 25% 로 둘지.
- **결정일**: 2026-08-07 (대표 결정)

---

## ADR-0019 — 포즈 자산 수치 정본 (SSOT 단일화) (2026-08-07)

- **문제**: 같은 대상을 가리키는 수치가 문서마다 달랐다 — 1,058 / 1,260 / 1,262 / 1,270.
  원인은 **1,058 이 두 가지 의미로 쓰인 것**이다(raw 루트 폴더 PNG 수 ↔ 25점 키포인트 사이드카 수).
  두 값이 우연히 같아 구분되지 않았다.
- **정본 정의** (2026-08-07 로컬 실측 + 2026-08-03 prod 실측):

| 수치 | 값 | 정의 |
|---|--:|---|
| raw PNG 파일 | **1,260** | `data/poses/raw/**.png` 전체 = 루트 1,058 + 동물 140 + 사랑 45 + 중복 17 |
| 태깅 대상 | **1,243** | 1,260 − `중복` 폴더 17 (사전 `subfolderTags.중복.skip=true`) |
| 25점 키포인트 사이드카 | **1,058** | 루트 폴더분만. 동물·사랑·중복 202장은 미보유 → 3점 자동 추정([ADR-0011c](#adr-0011c)) |
| prod DB `Resource(kind=pose)` | **1,262** | 적재 1,260 + 레거시 작품 참조 2 |

- **발견된 코드/정책 불일치**: `중복` 폴더 17장은 **태깅에서는 제외되지만 적재는 된다** —
  `filename-tagger` 는 skip 하는데 `scripts/ingest-poses.ts` 에는 폴더 제외 로직이 없다
  (skip 조건은 "이미 적재된 slug" 뿐). `llm-tagger.ts` 주석의 "중복 폴더 = 적재 제외"는 사실과 다르다.
  실질 영향은 태그 없는 자산 17건이 검색 인덱스에 남는 것(추천 노이즈, 인쇄·법적 위험 없음).
  → 정합화 방향(ingest 도 제외 / 태깅 정책 변경 / 현행 유지+문서화)은 [roadmap FOLLOWUP](roadmap.md) 로.
- **적용**: `CLAUDE.md` §1 자산 포맷 정책의 "보유 1,058개" 를 위 표 기준으로 정정.
  과거 ADR·핸드오프의 수치는 **작성 시점 기록**이므로 소급 수정하지 않는다.
- **결정일**: 2026-08-07
