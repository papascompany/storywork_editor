# 새 세션 시작 프롬프트 (2026-08-10 갱신 — MODE-04b 반영)
...
- **MODE-04a/04b 완료**: `Format.unit` 정본화(타입→zod→직렬화→ai-layout) · 좌표용 dpi 치환으로
  60여 호출처 무수정 · 인쇄 가드 구멍 3개 봉쇄 · **웹툰 세로 스트립 UI**(세그먼트별 캔버스 +
  가상화 라이브 창 3/2 + 활성 pin, 실기 검증 완료)
...
**키 등록 전이면 MODE-04c 부터** (MODE-04a/b 완료, 웹툰 시드는 액션 5번 재실행).
