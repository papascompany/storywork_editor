...
### 빌드 함정 3건 (FOLLOWUP-39/40 등록 + FOLLOWUP-23 재발)

| ID | 시도 | 증상 | 원인 | 수정 |
|---|---|---|---|---|
| 1 | `dcb1da1` | `editor-bubble:build TS6059 rootDir` | 신규 패키지 tsconfig.build.json 에 rootDir/paths/tsBuildInfoFile 누락 (commit a07dad3 패턴 미적용) | editor-text 와 동일한 13줄 설정 적용 → FOLLOWUP-39 등록 |
| 2 | `fb93c59` | `next/font/local` src not found | `apps/web/public/fonts/PretendardVariable.woff2` (2MB) 가 git 에 untracked — 로컬은 통과, Vercel 만 실패 | woff2 add + commit. 향후 git LFS → FOLLOWUP-40 등록 |
| 3 | `44727eb` | `TemplatePanel.tsx:37 (t: any)` next build 거부 | M3-04 e58730c 와 동일 — admin lint vs next build lint 불일치 (3번째 동일 함정) | RawTemplate type + unknown 가드 → FOLLOWUP-23 재발, ESLint 통일 정식 fix 권장 |
