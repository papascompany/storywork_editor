...
> 이 세션 = **SCRIPT-KO-04 + 제품 결정 3건 + LAYOUT-04/05 + MODE-01/02/03 + MODE-04a/b/c** 완료.
> 도중 모델 전환(Opus 5 → Fable 5, LAYOUT-05 착수 직전) — 체크포인트 대조 후 이어감.
> **08-09 새벽 prod 장애 발생·복구**(§1-⑥ · §5 함정 노트 필독).

## 0. 한 줄 상태

MODE-04c 까지 완료 · **prod 마이그레이션 적용 완료**(대표님 실행, FOLLOWUP-68 이력 정리 겸용 —
...
### ⑩ MODE-04c 웹툰 개방 정합 (08-10)
- **`Project.mode` 는 판형 파생값** — `format.unit` 에서 도출하고 클라이언트 입력은 무시한다.
  `save` 신규/업데이트 + `full-pipeline` 신규/업데이트 4개소 배선
- **업데이트 경로가 진짜 구멍**: `save` 가 formatId 는 바꾸면서 mode 는 안 고쳤다 →
  판형만 px 로 바뀌고 mode 는 pod 로 잔존하는 모순
- `/api/projects/[id]` 에 mode + 판형 실치수(unit 포함) 추가 → `FALLBACK_FORMAT` 을
  구버전 응답 대비 최후 폴백으로 강등(종전에는 무조건 128×182@350 로 열렸다)
- 웹툰 게이트는 유지 — px 생성 400. 개방은 MODE-05 와 한 세트
...
- 🟢 **MODE-05** 웹툰 이미지 export(시퀀스/롱 이미지) — **웹툰 게이트 개방의 마지막 관문**.
  완성 시 모달 "준비 중" 해제 + 프리셋 isActive + 서버 px 가드 해제가 한 세트.
  주의: 현재 `exportPng` 는 캔버스 엘리먼트가 뷰포트/세그먼트×zoom 이라 1:1 이 아니다 —
  판형 실측 렌더용 전용 패스가 필요하다
