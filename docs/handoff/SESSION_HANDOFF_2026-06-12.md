...
| FOLLOWUP-COVER-02 | (HEAD) | 표지 편집기 소비 — DB FormatPicker + 표지 페이지 + settings.cover |
...
- **P1 (표지 후속)**: 표지 페이지 PDF 출판 통합 — pdf-engine 이 cover 독립 치수 페이지를 어떻게 다룰지(현재 본문 치수 가정). preflight 표지 프로필.
