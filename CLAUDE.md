...
> **자산 포맷 정책 (요약)**
> - **1차(현재)**: PNG 마스터 — 투명 배경 RGBA. **raw 1,260장**(대부분 750×750) — 태깅 대상 1,243(`중복` 17 제외) · 25점 키포인트 사이드카 1,058(루트분) · prod DB 1,262행(적재 1,260 + 작품 참조 2). 수치 정본은 [ADR-0019](docs/architecture/decisions.md#adr-0019--포즈-자산-수치-정본-ssot-단일화-2026-08-07)
...
| 커뮤니티 | 공모전 / 뽐내기 / SNS 공유 |
| 교육 수강생 | 대본만으로 웹툰 만들기를 배우는 교육 과정(B2B 기관 · B2C 온라인) |
| 디자이너(공급자) | 캐릭터·포즈·배경 자산을 마켓에 등록·판매하고 수익 공유(수수료 25%) |
...
7. **교육 커리큘럼 & 디자인 마켓플레이스** — 제작 노하우 교육 과정(B2B/B2C) + 외부 디자이너 자산 거래·수익공유(수수료 25%) · *AI로 혁신한 자사 제작공정의 외부화* (M11/SW-BIZ)
8. **(로드맵)** AI 자동 더빙 / AI 영상 생성
...
### 1.4 수익 모델 (사업화 — 「혁신 소상공인 AI 활용지원사업」 연계)
- **기반 수익**: POD 인쇄 출판(건당 마진) + 크리에이터 구독(마이데이터, 연 ARPU 18만)
- **확장 수익**: 교육 커리큘럼(B2B 기관·B2C 온라인) + 디자인 마켓플레이스 수수료(25%) — 모두 *"AI로 혁신한 자사 제작공정의 **외부화**"* 로 포지셔닝(별도 플랫폼 신사업 아님, 취지 이탈 컷오프 방지)
- **3개년 보수 추정**: Y1 ≈ 5,300만 → Y2 ≈ 2.0억 → Y3 ≈ 4.6억 (현재 유료고객 0·결제 미구현에서 출발, 결제·온보딩 가동 전제). 상세 [roadmap SW-BIZ](docs/architecture/roadmap.md) · 근거 [ADR-0016](docs/architecture/decisions.md)
...
### M11 — 사업화 확장: AI 실가동 + 교육 + 디자인 마켓플레이스 (혁신소상공인 AI 사업 연계)
- AI 실가동·정밀화 시퀀스(mock→실 임베딩 · 라이브 LLM · 채택률 재측정) + 교육 커리큘럼(B2B/B2C) + 디자인 마켓플레이스(수수료 25%·정산)
- DoD: 결제·온보딩 가동 + 디자이너 온보딩 Y1 30명 + 자동배치 채택률 baseline→target
- 작업 분해: [roadmap SW-BIZ](docs/architecture/roadmap.md) · 결정: [ADR-0016](docs/architecture/decisions.md)
...
**최신 Handoff(항상 여기부터): [docs/handoff/SESSION_HANDOFF_2026-08-08.md](docs/handoff/SESSION_HANDOFF_2026-08-08.md) · 새 세션 시작: [docs/handoff/RESUME_PROMPT.md](docs/handoff/RESUME_PROMPT.md)**_
